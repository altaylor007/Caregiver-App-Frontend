-- Deactivated caregivers (status = 'inactive') no longer receive message/mention SMS. sms_enabled/sms_only_mentions/sms_shift_reminders are left untouched so reactivation restores prior SMS behavior automatically.
